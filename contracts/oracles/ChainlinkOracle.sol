...
contract ChainlinkOracle is PriceOracle {
    ...
    /// @notice Get the underlying price of a listed kToken asset
    ...
    function getPrice(KToken kToken) internal view returns (uint256 price) {
        EIP20Interface token = EIP20Interface(
            KErc20(address(kToken)).underlying()
        );
    ...
    /// @param symbol The symbol of the kToken's underlying token to set the feed for
    ...
    /// @param symbol The symbol of the kToken's underlying token to get the feed for
